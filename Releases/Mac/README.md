...
- 在波形上点击或拖动红色播放头定位，支持播放、暂停、停止，并可从当前剪切点直接试听
...
- 导入后按需测量 BPM：多片段节奏投票、节拍时间戳稳健回归、半速/倍速候选和可靠度提示
- 对可能变速的音乐在结果波形上标记连续 BPM 分段，并展示时长加权平均值、可信度与实际分析范围
- 拖入或选择 NCM 后在应用内确认，静默转换为 WAV 并载入，可随后测量 BPM
- 毛玻璃软件界面与动态流体背景，支持持久化明暗主题、手动关闭动效并遵循系统“减少动态效果”设置
- Windows 使用可拖动的自绘标题栏与原生圆角窗口（最大化时自动贴合屏幕）；macOS 使用原生标题栏与交通灯窗口控件
- 动态雾化背景随主题切换：亮色为蓝白，暗色为黑紫；支持系统“减少动态效果”和界面内动效开关
...
1. 检查兼容的 64 位 Python 3.10 至 3.13。
...
应用发生异常时会把 Python、Qt、后台音频任务和嵌入式界面错误写入 `~/Library/Logs/XMaoMusic OffsetEditor/application.log`。提交问题时请同时提供该日志与 macOS 生成的 `.ips` 文件。
