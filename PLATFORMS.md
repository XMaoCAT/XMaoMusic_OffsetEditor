...
XMaoMusic OffsetEditor 提供 Windows 和 macOS 两套独立的纯净版目录。两套版本使用相同的本地 HTML 界面、音频处理逻辑、格式转换能力、BPM 检测和 NCM 流式解密实现。
...
- `Core0/ncm-core.exe` 已包含在 Windows 纯净版中，NCM 转换不会弹出第三方窗口。
...
纯净版不包含虚拟环境、依赖下载缓存、用户输出音频、运行日志、Python 缓存、浏览器测试数据或 PyInstaller 中间目录。首次启动时，各平台会在自己的软件目录中创建本地运行环境。
